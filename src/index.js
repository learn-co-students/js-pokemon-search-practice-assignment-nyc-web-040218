document.addEventListener("DOMContentLoaded", function(event) {
  ...
  class Pokemon {
    ...
    render() {
      ...
      para.innerHTML = this.name;

      const frontImg = document.createElement("img");
      frontImg.setAttribute("src", this.frontImage);
      frontImg.setAttribute("style", "position:relative; top:0; left:0");
      const backImg = document.createElement("img");
      backImg.setAttribute("style", "position:absolute; top:30; left:70");
      backImg.setAttribute("src", this.backImage);

      listItem.appendChild(para);
      listItem.appendChild(frontImg);
      listItem.appendChild(backImg);
      pokemonList.appendChild(listItem);
    }
  }

  input.addEventListener("keyup", function(event) {
    while(pokemonList.hasChildNodes()) {
      pokemonList.removeChild(pokemonList.lastChild);
    }

    pokemons.pokemons.forEach(function(pokemonObj) {
      if(pokemonObj.name.indexOf(input.value) !== -1) {
        const pokemon = new Pokemon(pokemonObj.name, pokemonObj.sprites.front, pokemonObj.sprites.back);
        pokemon.render();
      }
    });
  });
